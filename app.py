"""
Voice-Enabled CSV Assistant Web App - Fixed for Cloud Deployment
...
class CSVAnalyzer:
    ...
    def load_data_from_content(self, csv_content: str) -> bool:
        try:
...
class VoiceCSVWebApp:
    def __init__(self):
        ...
        # Store analyzers in memory (consider Redis for production)
        self.analyzers = {}
    ...
    def setup_routes(self):
        ...
        def voice_query():
            try:
                ...
                if not session_id or not session.get('csv_loaded'):
                    return jsonify({'success': False, 'error': 'No CSV file loaded. Please upload a CSV first.'})
                
                # Get or recreate analyzer
                analyzer = self.get_or_create_analyzer(session_id, session.get('csv_content'))
                if not analyzer:
                    return jsonify({'success': False, 'error': 'Failed to initialize analyzer. Please re-upload your CSV.'})
        ...
        @self.app.route('/session_status', methods=['GET'])
        def session_status():
            """Check if CSV is loaded in session"""
            session_id = session.get('session_id')
    ...
    <script>
        ...
        async function checkSessionStatus() {
            try {
                ...
                if (result.csv_loaded && result.filename) {
                    csvUploaded = true;
                    textInput.disabled = false;
                    sendButton.disabled = false;
                    voiceStatus.textContent = `CSV loaded: ${result.filename}`;
